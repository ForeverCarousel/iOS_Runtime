...
{
//	printf("CXL- Load_Images:%s \n",path);
	//cxl-mark : 只调试runtime库的加载过程
	int cxlResult = strcmp(path, "/Users/chenxiaolong/Library/Developer/Xcode/DerivedData/objc-dazlisygpdlauiapxyrxxpjhisvr/Build/Products/Debug/libobjc.A.dylib");
	if (cxlResult == 0) {
		printf("CXL - 加载本地libobjc.A.dylib");
	}
...
static void schedule_class_load(Class cls)
{
	...
	//cxl-mark : 递归调用保证首先调用到根父类的+load方法
...
void prepare_load_methods(const headerType *mhdr)
{
	...
	//cxl-mark : 添加类的load方法到call load队列
    classref_t *classlist = 
	...
	//cxl-mark : 调用分类的load方法到call load队列

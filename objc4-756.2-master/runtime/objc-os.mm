...
//cxl-mark : 该方法是runtime库被加载之后执行的第一个方法 libSystem在library初始化之前 通过动态连接器注册image的通知
void _objc_init(void)
{
    ...
    // fixme defer initialization until an objc-using image is found?
    environ_init();//cxl-mark : 环境变量初始化
    tls_init();//cxl-mark : (thread local storage)线程本地缓存初始化
    static_init();//cxl-mark : 静态构造器初始化
    lock_init();//cxl-mark : 锁初始化
    exception_init();//cxl-mark : 异常处理器初始化
	
	//cxl-mark : map_images: 保存所有镜像文件的映射  load_images: 保存加载镜像文件的指针 unmap_image: 保存取消映射的处理函数指针
	//cxl-mark : map_images定义在Source/objc-runtime-new.m中
	/*cxl-mark : 注册了回调方法 当该image被mapped或者load 或者被unmap时会回调出来
